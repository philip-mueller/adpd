...
def load_cxr14_dataset(mode: str):
    """
    ...
    if not os.path.exists(CXR8_DIR):
        log.info(f"Dataset not found, downloading to {CXR8_DIR}")
        download_cxr14(CXR8_DIR)

    prepare_cxr14(CXR8_DIR)

    df = pd.read_csv(os.path.join(CXR8_DIR,
    ...
    if mode == 'train':
        ...
        for c in CLASSNAMES:
            df[f'nih_{c}'] = [1.0 if c in annot else 0.0 for annot in class_annotations]
    else:
        # (N, M_i, (x, y, h, w, c))
        ...
        df['nih_bboxes'] = annotations

    with Pool(min(12, os.cpu_count())) as pool:
        files = pool.map(
            partial(prepend_path, path=CXR8_DIR),
            df['file_name']
        )
...
def prepare_cxr14(path: str = CXR8_DIR):
    """
...
def download_cxr14(path: str = CXR8_DIR):
