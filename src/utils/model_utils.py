...
def update_config(source_config: BaseModelConfig, override_config):
    ...
    # for config_attr, config_attr_value in config.items():
    #     if hasattr(source_config, config_attr ):
    #         raise ValueError(f'Invalid config update, "{config_attr}" is not a valid config attribute')
    #     old_value = getattr(source_config, config_attr)

    #     if old_value == config_attr_value:
    #         continue  # config attribute was not changed
    #     elif config_attr in source_config.MODIFYABLE_ATTRIBUTES:
    #         # valid update
    #         setattr(source_config, config_attr, config_attr_value)
    #     else:
    #         raise ValueError(
    #             f'Invalid config update, trying to change unmodifyable attribute "{config_attr}" from "{old_value}" to "{config_attr_value}".'
    #             f'Modifyable attribute: {list(source_config.MODIFYABLE_CONFIGS)}')
...
class BaseModel(nn.Module):
    ...
    def __init__(self, config: Union[BaseModelConfig, dict]) -> None:
        ...
        if config.model_module is not None:
            assert module_name == config.model_module, f'{module_name} != {config.model_module}'
        else:
...
class ModelRegistry:
    ...
    def __init__(self, model_module: types.ModuleType) -> None:
        ...
        def register_model_class(model_cls):
            ...
            cs.store(name=model_cls_name, group=f'model/{module_name}', node=config_cls(model_module=module_name, model_class=model_cls_name))
        ...
        for (_, class_module_name, _) in iter_modules([package_dir]):
...
def instantiate_model(
    ...
    **kwargs) -> BaseModel:
    ...
    if isinstance(model_module, str):
        model_module = f'model.{model_module}'
...
def load_model_by_name(model_name: str, model_component: Optional[str] = None, step: Optional[int] = None, 
...
def load_model_from_checkpoint(checkpoint_path: str, config_override: Union[BaseModelConfig, dict, None] = None, return_dict=False):
    ...
    model = instantiate_model(config_dict, config_override=config_override)
